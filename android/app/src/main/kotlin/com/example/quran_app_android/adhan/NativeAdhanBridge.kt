...
object NativeAdhanBridge : MethodChannel.MethodCallHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            "schedulePrayerTimes" -> {
                val timesMap = call.arguments as? Map<String, Long>
                val context = appContext ?: return result.error("no_context", "Context مفقود", null)
                if (timesMap == null) return result.error("invalid_args", "مواعيد غير صحيحة", null)
    ...
    private fun saveLocation(context: Context?, lat: Double, lng: Double) {
        ...
        Log.i("NativeAdhanBridge", "📍 تم حفظ الموقع ($lat,$lng)")
    }
